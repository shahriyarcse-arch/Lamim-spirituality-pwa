...
const App = {
  ...
  async init() {
    ...
    if (!DB.rawGet('lamim_cache_cleared_v36')) {
      ...
      if (typeof caches !== 'undefined') {
        caches.keys().then(keys => {
          keys.forEach(key => { try { caches.delete(key); } catch(e) { console.warn('[App] Cache delete failed:', e); } });
        });
      }
    ...
    try {
      localStorage.setItem('lamim_settings', JSON.stringify(settings));
    } catch (e) { console.warn('[App] Settings save failed:', e); }
    // Sync theme icon
